...
int memo[MAXN][MAXN][11][2];
int n,m,k;
string a,b;

int dp(int i, int j, int k, int started){
    if(memo[i][j][k][started] != -1)return memo[i][j][k][started];
    else if(i == n || j == m)return memo[i][j][k][started] = (k!=0)?-INF:0;
    else if(k == 0 && started == 0) return memo[i][j][k][started] = 0;
    else{
        auto &r = memo[i][j][k][started]; r = -INF;
        if(a[i] == b[j]){
            if(started == 0){
                r = max(r, dp(i+1, j+1, k-1, 1) + 1);
                r = max(r, dp(i+1, j+1, k-1, 0) + 1);
            }else if(started == 1){
                r = max(r, dp(i+1, j+1, k, 1) + 1);
                r = max(r, dp(i+1, j+1, k, 0) + 1);
            }
        }
        r = max(r, dp(i,j+1, k, 0));
        r = max(r, dp(i+1,j, k, 0));
        return r;
    }
}
