...
void dfs(int cur, int par){
    if(iscat[cur])catCount[cur]++;
    else{
        catCount[cur] = 0;
    }
    mxCount[cur] = maX(catCount[cur], mxCount[par]);
    foreach(v, g[cur]){
        if(*v != par){
            catCount[*v]=catCount[cur];
            dfs(*v,cur);
        }
    }
}
...
int main(){
    ...
    dfs(1,0);
    int c=0;
    forall(i, 1, N+1)if(i!=1 && sz(g[i])==1 && mxCount[i] <= M){c++;}
    printf("%d\n",c);
}