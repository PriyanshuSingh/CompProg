...
#ifdef DEBUG
#define METHOD_NAME                  cerr<<__PRETTY_FUNCTION__<<":"<<__LINE__<<endl;
...
int main(){
    ...
    int d,h,v,e;
    cin>>d>>h>>v>>e;
    ld vol_inc_rate = acos(-1.0)*d*d*0.25*e;
    ld vol_dec_rate = v;
    if((vol_inc_rate - vol_dec_rate)*1e4 + acos(-1.0)*d*d*0.25*h > EPS)cout<<"NO\n";
    else{
        cout<<"YES\n";
        cout<<setprecision(10)<<(acos(-1.0)*d*d*0.25*h)/(-vol_inc_rate + vol_dec_rate)<<endl;
    }
}