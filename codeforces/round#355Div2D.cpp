...
struct Node{
    ...
    bool operator< (const Node other) const {
        if(value == other.value && x == other.x)return y < other.y;
        else if(value == other.value)return x < other.x;
...
int main(){
    ...
    forall(i, 0, n)forall(j, 0, m){
        ...
        if(room[i][j] == 1){
            dp[1].pb({i+j, i, j});
        }else dp[room[i][j]].pb({INF, i, j});
    }
    forall(i, 2, p+1){
        if(dp[i-1].size() * dp[i].size() >= n*m){
            bfs.clear();sort(all(dp[i-1]));
            bfs.pb(dp[i-1][0]);
            forall(i, 0, n)forall(j, 0, m)dis[i][j]=INF;
            dis[dp[i-1][0].x][dp[i-1][0].y] = dp[i-1][0].value;
            int pt = 1;
            int j = 0;
            while(j < bfs.size()){
                Node top = bfs[j++];
                while(pt < dp[i-1].size() && dp[i-1][pt].value <= top.value)bfs.pb(dp[i-1][pt++]);
                forall(k, 0, 4){
                    int new_x = top.x + fx[k][0];
                    int new_y = top.y + fx[k][1];
                    if(new_x >=0 && new_x < n && new_y >= 0 && new_y < m){
                        if(dis[new_x][new_y] > top.value + 1){
                            dis[new_x][new_y] = top.value + 1;
//                            trace1(dis[new_x][new_y]);
                            bfs.pb({dis[new_x][new_y], new_x, new_y});
                        }
                    }
                }
            }
...
//                trace1(dis[v->x][v->y]);
            ...
            foreach(v, dp[i]){
                foreach(w, dp[i-1]){
                    v->value = min(v->value, w->value + abs(w->x - v->x) + abs(w->y - v->y));
                }
            }
        }
    }
    cout<<dp[p][0].value<<endl;
    return 0;
}