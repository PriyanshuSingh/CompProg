...
void dfs(int cur, int par, ll sum){
    if(sum > a[cur])ans += size[cur];
    else{
        for(auto child: tree[cur]){
            if(child.F != par){
                if(cur == 1)dfs(child.F, cur, child.S);
                else dfs(child.F, cur,  max(child.S, sum + child.S));
            }
        }
    }    
}
