...
void add(string s, set<string> &ans){
    reverse(all(s));
//    trace1(s);
    ans.insert(s);
}
...
int main(){
    ...
    if(s.length() > 7){
        dp2[1] = true;
        dp3[2] = true;
        add(s.substr(0,2),ans);
        add(s.substr(0,3),ans);
    }else if(s.length() == 7){
        dp2[1] = true;
        add(s.substr(0,2), ans);
    }
    forall(i, 3, s.length() - 5){
        dp2[i] = dp3[i-2] || (dp2[i-2] && s.substr(i-3,2) != s.substr(i-1, 2));
        dp3[i] = dp2[i-3] || (dp3[i-3] && s.substr(i-5,3) != s.substr(i-2, 3));
        if(dp2[i])add(s.substr(i-1,2), ans);
        if(dp3[i])add(s.substr(i-2,3), ans);
    }
    cout<<ans.size()<<endl;
    foreach(v, ans)cout<<*v<<endl;
    return 0;
}