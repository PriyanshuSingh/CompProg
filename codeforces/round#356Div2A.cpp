...
int main(){
    int t[10];
    forall(i, 0, 5)cin>>t[i];
    sort(t, t+5);
    int ans=0;
    forall(i, 0, 5)ans+=t[i];
    for(int i = 4; i>=1; i--){
        if(t[i] == t[i-1]){
            if(i-2 >=0 && t[i-2] == t[i]){
                cout<<ans-t[i]*3<<endl;return 0;             
            }else{
                if(i == 4 && t[0] == t[1] && t[1] == t[2] && 3*t[0] > 2*t[4] ){
                    cout<<ans - 3*t[0]<<endl;return 0;
                }else{
                    cout<<ans - 2*t[i]<<endl;return 0;
                }
            }
        }
    }
