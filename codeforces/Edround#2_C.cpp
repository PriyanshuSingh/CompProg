...
int main(){
    ...
    fill(freq,0);
    int len = strlen(str);
    forall(i, 0, len)freq[str[i]]++;
    int i = 'a';
    int j = 'z';
    bool odd = false;
    char oddin;
    while(i <= j){
//        trace2((char)i,(char)j);
        while(i<j && freq[i] % 2 == 0)i++;
        while(j>i && freq[j] % 2 == 0)j--;
        if(i == j && freq[i] % 2 == 0){
            odd = false;
            break;
        }else if(i == j && freq[i] % 2 != 0){
            odd = true;
            oddin = i;
            break;
        }else{
    ...
    forall(i, 'a', 'z'+1 ){
//        trace2((char)i, freq[i]);
        forall(j, 0, freq[i]/2)if(freq[i]!=0)printf("%c",(char)i);
    }
    //trace2(oddin,odd);
    if(odd)printf("%c",oddin);
    for(int i=  'z'; i>='a'; i-- ){
        forall(j, 0, freq[i]/2)if(freq[i]!=0)printf("%c",(char)i);
    }
    printf("\n");
