...
int solve(int r, int c, int k){
    ...
    else if(k > r*c)return memo[r][c][k] = INF;
    else if(k == r*c || k == 0)return memo[r][c][k]=0;
    else{
        memo[r][c][k] = INF;
        forall(i, 1, r/2 + 1){
            forall(j, 0, k+1){
                memo[r][c][k] = min(memo[r][c][k], solve(i, c, j) + solve(r-i, c, k-j) + c*c);
            }
        }
        forall(i, 1, c/2 + 1){
            forall(j, 0, k+1){
                memo[r][c][k] = min(memo[r][c][k], solve(r, i, j) + solve(r, c-i, k-j) + r*r);
            }
        }
        return memo[r][c][k];
    }
