...
const int MSET = 1<<18;
const int MAXN = 20;
long double dp[MAXN][MSET];
long double p[MAXN][MAXN];
int n;
long double getans(int s, int left){
    if(dp[s][left] > -0.5)return dp[s][left];
    if(left == (1<<s))return dp[s][left] = 1.0;
    else{
        int c= 0;
        dp[s][left] = 0;
        forall(i, 0, n){
            if(i != s){
                if(checkbit(left, i)){
                    ...
                    int ci = 0;
                    forall(j, 0, n){
                        if(i != j && checkbit(left, j))aa += p[j][i];ci++;
                    }
                    aa /=ci;
                    dp[s][left] += getans(s, left ^ (1 << i)) * aa;
                    dp[s][left] += getans(i, left ^ (1 << s)) * p[s][i];
                }
            }
        }
        dp[s][left] /= c * 2;
...
int main(){
    cin>>n;
    forall(i, 0, n)forall(j, 0, n)cin>>p[i][j];
    forall(i, 0, n)forall(j, 0, MSET)dp[i][j] = -1;
    cout<<setprecision(10)<<getans(0, (1<<n)-1)<<endl;
}