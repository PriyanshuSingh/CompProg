...
template<typename T> ostream& operator<<(ostream &s,set<T> t){s<<"[";foreach(v, t){s<<*v;__typeof(v) c = v;c++;if(c != t.end())s<<", ";}s<<"]";return s;}
...
int main(){
    ...
    forall(i, 1, N){
        s(a);
//        trace1(num);
        set<int>::iterator r = num.lower_bound(a);
        if(r != num.end() && lft.count(*r) == 0){
            lft[*r] = a;
            pi(*r);putchar(' ');
        }else{
            auto l = --r;
