...
long long dis(ll x1, ll y1, ll x2, ll y2){
...
int main(){
    ...
    if(dis(x1, y1, x2, y2) >= (r1 + r2)*(r1+r2)){
        printf("%.10Lf",ans);
    }else{
        long double rd1 = r1;
        long double rd2 = r2;
        ...
        long double yd1 = y1;
        long double yd2 = y2;
        long double dist = sqrtl((xd2-xd1)*(xd2-xd1) + (yd2-yd1)*(yd2-yd1));
        trace3(dist, r1, r2);
        if(rd1 + dist <= rd2){
            ans = M_PI * rd1 * rd1;
        }else if(rd2 + dist <= rd1){
            ans = M_PI * rd2 * rd2;
        }else{
            long double alpha1 = acosl((rd1*rd1 + dist*dist - rd2*rd2)/(2*rd1*dist));
            long double alpha2 = acosl((rd2*rd2 + dist*dist - rd1*rd1)/(2*rd2*dist));
            long double aaa = rd1 * rd1 * sinl(alpha1) * cosl(alpha1);
            long double bbb = rd2 * rd2 * sinl(alpha2) * cos(alpha2);
            trace3(alpha1, alpha2, aaa);
            long double ar1 = alpha1 * rd1 * rd1;
            long double ar2 = alpha2 * rd2 * rd2;
            trace2(ar1, ar2);
            ans = ar1 + ar2 - aaa - bbb;
            trace2(aaa, bbb);
        }
