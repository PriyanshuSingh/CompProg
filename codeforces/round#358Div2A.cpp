...
int main(){
    ...
    ll right[5];
    ll n,m;cin>>n>>m;
    left[0] = left[1] = left[2] = left[3] = left[4] = n/5;
    right[0] = right[1] = right[2] = right[3] = right[4] = m/5;
    if(n%5 != 0)forall(i, 1, n%5+1)left[i]++;
    if(m%5 != 0)forall(i, 1, m%5+1)right[i]++;
    ll ans = 0;
    forall(i, 0, 5){
        ans += left[i] * right[(5-i)%5];
    }
    cout<<ans<<endl;
    return 0;
}