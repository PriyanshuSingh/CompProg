...
map<int , int> dfs(int cur, int par){
    ...
    ans[cur] = vertex_color[cur];
    max_freq[cur] = 1;
    
    curmap[vertex_color[cur]] = 1;
    
    forall(i, 0, graph[cur].size()){
        if(graph[cur][i] != par){
            
            int to = graph[cur][i];
            tmpmap = dfs(to, cur);
            
            if(curmap.size() < tmpmap.size()){curmap.swap(tmpmap);max_freq[cur] = max_freq[to]; ans[cur] = ans[to];}
            
            foreach(v, tmpmap){
                int a = (curmap[v->F] += v->S);
                if(a > max_freq[cur]){
                    max_freq[cur] = a;
                    ans[cur] = v->F;
                }else if(a == max_freq[cur]){
                    ans[cur] += v->F;
                }
            }
...
//    forall(i, 0, MAXN)graph[i].clear();
    int N;cin>>N;
//    fill(ans, 0);
//    fill(max_freq, 0);
    forall(i, 0, N)cin>>vertex_color[i+1];
//    fill(color_freq, 0);
    forall(i, 0, N-1){
        int x,y;cin>>x>>y;
        graph[x].pb(y);
