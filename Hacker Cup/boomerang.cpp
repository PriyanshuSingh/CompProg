...
int main(){
    ...
    forall(t, 1, T+1){
        int n;s(n);
        forall(i, 0, n){s(pts[i].F);s(pts[i].S);}
        ll cc=0,ct=1,p=0;
        forall(i, 0, n){
            forall(j, i+1, n){
                int xlen,ylen;
                xlen = pts[i].F - pts[j].F;xlen=(xlen<0)?-xlen:xlen;
                ylen = pts[i].S - pts[j].S;ylen=(ylen<0)?-ylen:ylen;
                arr[p++] = ((ds){xlen*xlen+ylen*ylen,pts[i].F,pts[i].S});
                arr[p++] = ((ds){xlen*xlen+ylen*ylen,pts[j].F,pts[j].S});
        ...
        forall(i, 1, p){
            //trace3(arr[i].len, arr[i].x, arr[i].y);
            if(arr[i-1].len == arr[i].len && arr[i-1].x == arr[i].x && arr[i-1].y == arr[i].y){
                ct++;
            }else{
                ct = (ct*(ct-1))>>1;
                cc+=ct;
