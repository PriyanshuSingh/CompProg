...
int main(){
    ...
    int t;cin>>t;while(t--){
        int n;cin>>n;forall(i,0,n)cin>>A[i];
        ll max_sum = LONG_MIN;
        ll cur_sum = 0, cur_min = 0, last_sum = 0;
        bool pos = false;forall(i, 0, n)if(A[i]>0)pos = true;
        if(!pos){
            forall(i, 0, n)max_sum = max(A[i], max_sum);
        }else{
            forall(i, 0, n){
                                    trace4(last_sum, cur_min, A[i], cur_sum);
                if(A[i] > 0)cur_sum += A[i];
                else{
                    if(last_sum - cur_min <= -A[i]){
                        cur_sum -= last_sum;
                        cur_sum += A[i];
                        cur_min = A[i];
                        last_sum = cur_sum;
                    ...
                    if(cur_sum - cur_min <= 0){cur_sum=0;cur_min=0;last_sum=0;}
                }
                max_sum = max(max_sum, cur_sum - cur_min);
    //            trace3(cur_sum, cur_min, cur_sum - cur_min);
            } 
            max_sum = max(max_sum, cur_sum - cur_min);
        }
        cout<<max_sum<<endl;       
