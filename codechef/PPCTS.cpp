...
ll getAns(int xx, int aX[], ll bX[], int N){
    int low = 0,high=N;while(low < high){
        int mid = (low + high)>>1;
        if(xx > aX[mid])low = mid; else high = mid;
        if(low + 1 == high)break;
    }
    int ind;
    if(aX[high] <= xx)ind = high;
    else if(aX[low] <= xx)ind = low;
    else ind = -1;
//    trace1(ind);
    ll sum1 = 0, sum2 = 0;if(ind >= 0){sum1 = bX[ind];}
    sum2 = bX[N] - sum1;return 1LL * xx * (ind + 1 - (N - ind)) - sum1 + sum2;
}

int main(){
    ios_base::sync_with_stdio(false);
    int n,q;cin>>n>>q;forall(i, 0, n){cin>>X[i]>>Y[i];}
    sort(X, X+n);sort(Y, Y+n);Xsum[0] = X[0];Ysum[0] = Y[0];forall(i, 1, n){
        Xsum[i] = Xsum[i-1] + X[i]; Ysum[i] = Ysum[i-1] + Y[i];
    }
//    traceArr(Xsum, n);traceArr(Ysum, n);
    int x=0,y=0;string s;cin>>s;forall(i, 0, s.length()){
        if(s[i] == 'D')y--;else if(s[i] == 'U')y++;else if(s[i] == 'R')x++; else x--;
        cout<<getAns(x, X, Xsum, n-1) + getAns(y, Y, Ysum, n-1)<<endl;
    }
    return 0;
}