...
int main(){
    ...
    while(T--){
        s(N);
        forall(i, 0, N){
            s(arr[i].A);s(arr[i].B);s(arr[i].C);
            if(arr[i].A < 0 || (arr[i].A == 0 && arr[i].B < 0)){
                arr[i].A = -arr[i].A;
                arr[i].B = -arr[i].B;
                arr[i].C = -arr[i].C;
            }
            int g = gcD(gcD(arr[i].A,arr[i].B),arr[i].C);
            arr[i].A /= g;arr[i].B /= g;arr[i].C /= g;
        }
        sort(arr, arr + N, cmp);
        int k=0;
        forall(i, 1, N)if(!equalLine(arr[k], arr[i]))arr[++k]=arr[i];
        ++k;
        forall(i, 0, k){
            //trace4(arr[i].A, arr[i].B, arr[i].C, i);
            int g= gcD(arr[i].A, arr[i].B);
            arr[i].A /= g;arr[i].B /= g;
        }
        sort(arr, arr + k, cmp);
        int mset=1,mxset=1;
