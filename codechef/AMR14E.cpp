...
void fill_sieve(){
    ...
    forall(i, 2, SQTN){
        if(!sieve[i]){
            prime.pb(i);
            for(int j=(i<<1); j<SQTN; j+=i)sieve[j]=true;
        }
    }
}
...
int main(){
    ...
    while(T--){
        s(A);s(B);
        c=0;
        forall(i, A, B+1)arr[i-A]=true;
        forall(i, 0, sz(prime)){
            int st = A/prime[i];
            if(st*prime[i] < A)st++;
            for(ll j=st; j*prime[i] <= B; j++){
                if(arr[j*prime[i]-A] == false)continue;
                int s = j;
                int k = 1;
                while(s != 1){
                    if(s%prime[i]==0){s /= prime[i]; k++;}
                    else {k=-1; break;}
                }
                if(k > 0 && !sieve[k+1])c++;
                arr[j*prime[i]-A] = false;
            }
        }
        forall(i, A, B+1)if(arr[i-A])c++;
        printf("%d\n",c);
    }    
    return 0;
}