...
int main(){
    ...
    int q;cin>>q;while(q--){
        int n,u,v;
        cin>>n>>u>>v;if(u<v)swap(u,v);
        int ulvl=0,vlvl=0;
        forall(i, 0, 32){if(checkbit(u, i))ulvl=i;if(checkbit(v, i))vlvl=i;}
        int uu = u;
        int diff =ulvl-vlvl,c = diff;while(diff--)uu>>=1;
        while(uu != v){uu>>=1;v>>=1;c++;}
        ll p2=1;while(c--)p2<<=1;
        cout<<(n - u%p2)/p2<<endl;
    }
}
