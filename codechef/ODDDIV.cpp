...
void preprocess(){
    ...
    vi prime;
    forall(i, 2, MAXN){
        if(isprime[i]){for(int j=2*i;j<MAXN;j+=i)isprime[j]=false;prime.push_back(i);}
    }
    for(int i=1; i<MAXN; i<<=1)ans[i]=1;
    forall(i, 1, prime.size())for(ll j = prime[i]; j < MAXN; j *= prime[i])ans[j] = ans[j/prime[i]]+j;
    forall(i, 3, MAXN){
        if(ans[i] == -1){
            ll cur = i;
            forall(j, 0, prime.size()){
                if(cur % prime[j] == 0){ll p=1;while(cur % prime[j] == 0){cur/=prime[j];p*=prime[j];} ans[i] = ans[p]*ans[cur];break;} 
            }
        }
    }
//    traceArr(ans, MAXN);
...
int main(){
    ...
    preprocess();    
    int t;cin>>t;
    while(t--){int l,r;cin>>l>>r;ll a=0; forall(i, l, r+1)a+=ans[i]; cout<<a<<endl;}
    return 0;
}
