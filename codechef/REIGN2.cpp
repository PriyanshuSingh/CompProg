...
ll DP(int i, int j){
    ...
    else if(i==0)return memo[i][j] = a[0].S;
    else if(i+1==j)return memo[i][j] = DP(i-1,j-1)+(1LL*(j-1)*a[i].F + (ll)a[i].S);
    else return memo[i][j] = maX((i+2>=j)?DP(i-1,j):0, DP(i-1,j-1)+(1LL*(j-1)*a[i].F + (ll)a[i].S));
}
...
int main(){
    ...
    while(T--){
        s(N);
        forall(i, 0, N)s(a[i].S);
        forall(i, 0, N)s(a[i].F);
        sort(a,a+N);reverse(a,a+N);
        fill(memo,-1);
        forall(i, 1, N+1)printf("%lld ",DP(N-1,i));
        printf("\n");
    }
}