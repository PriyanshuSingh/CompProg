...
const int MAXN = 5e4+10;int c[MAXN],A[MAXN];
int main(){
    fill(c,0);int t;cin>>t;while(t--){
        int n;cin>>n;forall(i, 0, n){cin>>A[i];c[i]=0;}
        sort(A, A+n);c[0]++;int pre=A[0],in = 0;forall(i, 1, n){
            if(pre != A[i]){pre = A[i];in++;}
            c[in]++;
        }
        priority_queue<int> pQ;
        forall(i, 0, in+1){pQ.push(c[i]);c[i]=0;}
//        auto it = pQ.begin(); for(; it != pQ.end(); it++){trace1(*it);}
        int ans = 0;while(!pQ.empty()){
           if(pQ.size() == 1){ans += pQ.top();pQ.pop();}
           else {int top = pQ.top();pQ.pop();int stop = pQ.top();pQ.pop();top--;stop--;if(top>0)pQ.push(top);if(stop>0)pQ.push(stop);ans++;}
        }
        cout<<ans<<endl;
    }
}