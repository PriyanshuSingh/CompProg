...
struct SegTree{
    ...
    SegTree(vi &arr){
        ...
        orv.assign(arr.size() << 2, 0);
        build(1, 0, arr.size()-1, arr);
    }
    void build(int cur, int s, int e, vi &arr){
        if(s == e){
            st[cur] = orv[cur] = arr[s];
        }else{
            int mid = (s+e)>>1, c1 = cur<<1, c2 = c1 | 1;
            build(c1, s, mid, arr); build(c2, mid+1, e, arr);
            st[cur] = min(st[c1], st[c2]);
            orv[cur] = orv[c1] | orv[c2];
        }
    }
    void update(int cur, int s, int e, int St, int En, int x){
        if(En < s || e < St)return;
        else if(St <= s && e <= En){
            for(int i=0; i<32; i++){
                if(checkbit(orv[cur], i) == 1 && checkbit(x, i) == 0)update(cur, s, e, i);
            }
        }else{
            int mid = (s + e)>>1, c1 = cur<<1, c2 = c1 | 1;
            update(c1, s, mid, St, En, x);
            update(c2, mid+1, e, St, En, x);
            st[cur] = min(st[c1], st[c2]);
            orv[cur] = orv[c1] | orv[c2];
        }
    }
    void update(int cur, int s, int e, int pos){
        if(s == e){
            st[cur] -= (1 << pos);
            orv[cur] -= (1 << pos);
        }else{
            int mid = (s+e)>>1, c1 = cur<<1, c2 = c1 | 1;
            if(checkbit(orv[c1], pos) == 1)update(c1, s, mid, pos);
            if(checkbit(orv[c2], pos) == 1)update(c2, mid+1, e, pos);
            st[cur] = min(st[c1], st[c2]);
            orv[cur] = orv[c1] | orv[c2];
        }
    }
    int query(int cur, int s, int e, int St, int En){
        if(En < s || e < St)return INF+10;
        else if(St <= s && e <= En)return st[cur];
        else{
            int mid = (s+e)>>1, c1 = cur<<1, c2 = c1 | 1;
            return min(query(c1, s, mid, St, En), query(c2, mid+1, e, St, En));
        }
    }
};
vi a;
int main(){
    ...
    while(q--){
        int t,l,r;cin>>t>>l>>r;
        l--;r--;
        if(t==0)cout<<st.query(1, 0, a.size()-1, l, r)<<endl;
        else {
            int x;cin>>x;
            st.update(1, 0, a.size()-1, l, r, x);
        }
    }
}