...
int solve(int i, int j){
    ...
    else if(i == 0 && j == 0)ans[0][0] = 0;
    else {ans[i][j] = 0;forall(k,1,3)if(j-k>=0)ans[i][j] |= 1 - solve(i,j-k);
    forall(k, 1, 4)if(i-k>=0)ans[i][j] |= 1 - solve(i-k,j);}
    return ans[i][j];
}
 
int main(){
    ios_base::sync_with_stdio(false);
    fill(ans,-1);solve(3,2);
    int t;cin>>t;while(t--){int n,m;cin>>n>>m;
        n--;m--;n %= 4;m %= 3;
        cout<<((ans[n][m] == 0)?"Vanya":"Tuzik")<<endl;
    }
    return 0;
}