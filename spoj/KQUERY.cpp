...
int ans[MAXQ];
int N,Q;
inline void set_bit(int id, int val){for(int j=id; j<=N; j += (-j)&j)BIT[j]+=val;}
inline int get_bit(int id){int c=0; for(int j=id; j>0; j -= (-j)&j)c+=BIT[j]; return c;}

bool cmpp(query a, query b){return a.k < b.k;}

int main(){
    fs(N);
    forall(i, 0, N+1)BIT[i]=0;
    forall(i, 0, N){fs(a[i].F);a[i].S=i+1;set_bit(i+1,1);}
    fs(Q);
    sort(a, a+N);
    forall(i, 0, Q){fs(q[i].l);fs(q[i].r);fs(q[i].k);q[i].ind=i;}
    sort(q,q+Q,cmpp);
    int cur = 0;
    forall(i, 0, Q){
        int k = q[i].k;
        int l = q[i].l;
        int r = q[i].r;
        while(a[cur].F <= k && cur < N)set_bit(a[cur++].S, -1);
        ans[q[i].ind] = get_bit(r) - get_bit(l-1);
    }
    forall(i, 0, Q)fastprint(ans[i]);
    return 0;
}